...
func NewCron(ctx *cli.Context, store store.Store) (*Cron, error) {
...
func (c *Cron) Start() {
	agentsRetention := c.ctx.String("maintenance-cleanup-agents-older-than")
	if agentsRetention != "" {
		c.setupStaleAgentsCleanup(agentsRetention)
	}
	logsRetention := c.ctx.String("maintenance-cleanup-pipeline-logs-older-than")
	if logsRetention != "" {
