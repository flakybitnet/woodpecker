...
func mkPod(step *types.Step, config *config, podName, goos string, options BackendOptions) (*v1.Pod, error) {
	...
	container, err := podContainer(step, config, options, podName, goos)
	if err != nil {
		return nil, err
	}
...
func podContainer(step *types.Step, config *config, options BackendOptions, podName, goos string) (v1.Container, error) {
	var err error
...
func podSecurityContext(sc *SecurityContext, secCtxConf SecurityContextConfig, stepPrivileged bool) *v1.PodSecurityContext {
	...
	if secCtxConf.User > 0 {
		user = newInt64(int64(secCtxConf.User))
	}
	if secCtxConf.Group > 0 {
		group = newInt64(int64(secCtxConf.Group))
	}
	if secCtxConf.FsGroup > 0 {
		fsGroup = newInt64(int64(secCtxConf.FsGroup))
	}
